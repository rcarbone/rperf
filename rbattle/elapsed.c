...
#if defined(ROCCO)
static void rocco (void)
{
  if (n2day (nsecs))
    sprintf (elapsed, "%ju days, %02ju:%02ju:%02ju.%03ju",
	     n2day (nsecs), n2hour (nsecs), n2min (nsecs), n2sec (nsecs), n2usec (nsecs));
  else if (n2hour (nsecs))
    sprintf (elapsed, "%02ju:%02ju:%02ju.%03ju", n2hour (nsecs), n2min (nsecs), n2sec (nsecs), n2usec (nsecs));
  else if (n2min (nsecs))
    sprintf (elapsed, "%02ju:%02ju.%03ju", n2min (nsecs), n2sec (nsecs), n2usec (nsecs));
  else if (n2sec (nsecs))
    sprintf (elapsed, "%2ju secs", n2sec (nsecs));
  else if (n2msec (nsecs))
    sprintf (elapsed, "%3ju msecs", n2msec (nsecs));
  else if (n2usec (nsecs))
    sprintf (elapsed, "%3ju usecs", n2usec (nsecs));
  else
    sprintf (elapsed, "%3ju nsecs", nsecs);
}
#endif /* ROCCO */
