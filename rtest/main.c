...
/* Program version */
#define _NAME_      "Run Unit Tests and Test Suite for Hash Tables"
#define _VERSION_   "0.1.0"

/* Default option (if none was specified) */
#define OPT_DEFAULT OPT_LIST_UT
...
static struct option lopts [] =
{
  ...
  /* Operations */
  { "unit-list",    no_argument,       NULL, OPT_LIST_UT      },
  { "suite-list",   no_argument,       NULL, OPT_LIST_TS      },
  ...
  if (argv)
    {
      switch (choice)
	{
	case OPT_LIST_UT: runit_print_these (argv);      break;
	case OPT_LIST_TS: rsuite_print_these (argv);     break;
...
static void _usage_ (char * progname, char * version, struct option * options)
{
  ...
  usage_item (options, n, OPT_LIST_UT,      "list Unit Tests");
  usage_item (options, n, OPT_LIST_TS,      "list Test Suite");
...
/* Display/Select/Execute Unit Tests or Test Suite */
int main (int argc, char * argv [])
{
  ...
  while ((option = getopt_long (argc, argv, sopts, lopts, NULL)) != -1)
	...
	case OPT_QUIET:        quiet   = true;                          break;
	  ...
	  /* Operations */
	case OPT_LIST_UT:      all = runit_all (); choice = option;     break;
	case OPT_LIST_TS:      all = rsuite_all (); choice = option;    break;
