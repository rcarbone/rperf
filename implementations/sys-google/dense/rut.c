...
static rtest_t ** choose (char * progname, char * included [], char * excluded [])
{
  ...
  rtest_t ** subset = included ? NULL : rut_all ();

  /* Nothing but these */
  while (names && * names)
    {
      rtest_t * rut = rut_valid (* names);
      if (! rut)
	{
	...
	subset = included ? arrmore (subset, rut, rtest_t) : arrless (subset, rut, rtest_t, NULL);
      names ++;
    }
...
static void doit (char * progname, unsigned choice, rtest_t * rargv [], unsigned items, unsigned runs, bool verbose, bool quiet)
{
  if (rargv)
    {
      switch (choice)
