...
/* Program version */
#define _NAME_      "Run Test Suite over this implementation"
...
static rtest_t ** choose (char * progname, char * included [], char * excluded [])
{
  ...
  rtest_t ** subset = included ? NULL : rsuite_all ();

  /* Nothing but these */
  while (names && * names)
    {
      rtest_t * rtest = rsuite_valid (* names);
      if (! rtest)
	{
  ...
  if (argv)
    {
      ...
      switch (choice)
	{
	case OPT_LIST:    rsuite_print_these (argv); break;
	case OPT_EXECUTE:
	  objs = mkobjs (items);            /* Initialize datasets needed by the suite */
	  rsuite_run_these (argv, items, objs);
	  rmobjs (objs);                   /* Free the datasets used by the test suite */

	  break;
	}
    }
  else
    printf ("%s: no Test Suite defined\n", progname);
}
...
/* Display/Select/Execute Test Suite */
