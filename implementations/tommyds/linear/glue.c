...
{
  kvcb_t * kv = arg;
  tommy_hashlin_node * node = data;
#if defined(ROCCO)
  // kv -> keys [kv -> i ++] = node -> key;
#else
  kv -> keys [kv -> i ++] = "rocco";
  printf ("i = %u - key = [%p]\n", kv -> i, ((tommy_hashlin_node *) data) -> key);
#endif /* ROCCO */
  return false;
}
...
{
  robj_t item;
  return tommy_hashlin_search (ht, cmp_str, & item, rht_python_hash (key));
}
...
void rht_foreach (rht_t * ht, rht_each_f * fn, void * data)
{
  ...
  tommy_hashlin_foreach_arg (ht, foreach_cb, & fun);
}
...
char ** rht_keys (rht_t * ht)
{
  ...
  tommy_hashlin_foreach_arg (ht, addkey_cb, & kv);
  return keys;
}
...
void ** rht_vals (rht_t * ht)
{
  ...
  tommy_hashlin_foreach_arg (ht, addval_cb, & kv);
  return vals;
}
