
//
// iterator.c
//
// Copyright (c) 2010 TJ Holowaychuk <tj@vision-media.ca>
//

#include "list.h"

/*
 * Allocate a new list_iterator_t. NULL on failure.
 * Accepts a direction, which may be TJ_LIST_HEAD or TJ_LIST_TAIL.
 */

list_iterator_t *
list_iterator_new(tj_list_t *list, list_direction_t direction) {
  tj_list_node_t *node = direction == TJ_LIST_HEAD
    ? list->head
    : list->tail;
  return list_iterator_new_from_node(node, direction);
}

/*
 * Allocate a new list_iterator_t with the given start
 * node. NULL on failure.
 */

list_iterator_t *
list_iterator_new_from_node(tj_list_node_t *node, list_direction_t direction) {
  list_iterator_t *self;
  if (!(self = LIST_MALLOC(sizeof(list_iterator_t))))
    return NULL;
  self->next = node;
  self->direction = direction;
  return self;
}

/*
 * Return the next tj_list_node_t or NULL when no more
 * nodes remain in the list.
 */

tj_list_node_t *
list_iterator_next(list_iterator_t *self) {
  tj_list_node_t *curr = self->next;
  if (curr) {
    self->next = self->direction == TJ_LIST_HEAD
      ? curr->next
      : curr->prev;
  }
  return curr;
}

/*
 * Free the list iterator.
 */

void
list_iterator_destroy(list_iterator_t *self) {
  LIST_FREE(self);
  self = NULL;
}
