...
#define _NAME_      "Time several List implementations and sort the results"
...
static rlsuite_t ** build_suite (char * progname, char * included [], char * excluded [])
{
  char ** names    = included ? included : excluded;       /* User-included items have priority over user-excluded        */
  rlsuite_t ** suite = included ? NULL : rlsuite_all ();   /* The suite of tests to run (nothing or everything but these) */
  ...
  while (names && * names)
    {
      ...
      if (t)
	suite = included ? arrmore (suite, t, rlsuite_t) : arrless (suite, t, rlsuite_t, NULL);
      else
	{
...
/* Evaluate and compare performances of list implementations */
int main (int argc, char * argv [])
{
  ...
  while ((option = getopt_long (argc, argv, sopts, lopts, NULL)) != -1)
	...
	case OPT_QUIET:        quiet   = true;                          break;
	  ...
	  /* List */
        case OPT_LIST_TESTS:   rlsuite_print_all ();                    goto bye;
  ...
  if (files)
    {
      ...
      rlsuite_t ** suite = build_suite (progname, enabled, disabled);
      if (suite)
	{
