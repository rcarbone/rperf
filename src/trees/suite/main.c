...
{
  return rsuite_handles (rtree_suite_argc (), rtree_suite_argv ());
}


static rsuite_t * rsuite_valid_all (char * name)
{
  return rsuite_valid (rtree_suite_argc (), rtree_suite_argv (), name);
}


static void rsuite_run_all (rsuite_t * suite [], unsigned items, rnode_t * nodes [])
{
  unsigned maxn = rsuite_maxn (rtree_suite_argc (), rtree_suite_argv ());
		  ...
		  unsigned items, unsigned loops, bool verbose, bool quiet)
{
  ...
  switch (choice)
    {
    case OPT_LIST: rsuite_prints (rtree_suite_argc (), rtree_suite_argv ()); break;
