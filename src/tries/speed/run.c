 ...
 */
static relapsed_t * run_single_test (rsuite_t * test, sw_t * sw,
				     unsigned loops,
				     unsigned items, rnode_t * elems [],
				     unsigned serial,
 ...
 */
sw_t ** run_suite (rsuite_t * suite [], sw_t * plugins [],
		   unsigned loops, unsigned items,
		   bool verbose, bool quiet)
{
  /* Initialize variables needed to run the suite */
  rnode_t ** nodes = mknodes (items);           /* Initialize datasets needed by the suite */
  ...
  rsuite_t ** test;                             /* Iterator over the table of tests to run */
  ...
  printf ("\n");
  printf ("Dataset  : unique generic pointers (void *) as nodes\n");
  ...
  while (test && * test)
      ...
      if (torun)
	{
	   ...
	   *  * run a single test for the given number of times and take min/avg/max times elapsed for its execution
	   */
	  t1 = nswall ();
	  for (i = 0; i < loaded; i ++)
	    {
	      ...
	      if (rplugin_implement (sw -> plugin, (* test) -> name))
		{
		  ...
		  relapsed_t * result = run_single_test (* test, sw, loops, items, nodes, i + 1, maxn, quiet);
  ...
  /* Free the nodes used by the test suite */
  rmnodes (nodes);
