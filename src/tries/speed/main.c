...
typedef enum
{
  ...
  /* Finger */
  OPT_ADD_TEST     = 'i',    /* Include   */
  OPT_DEL_TEST     = 'e',    /* Exclude   */

  /* Actions */
  OPT_EXECUTE      = 'x',    /* Execute   */
  OPT_LIST_TESTS   = 's',    /* List      */
...
/* Evaluate and compare performances of implementations */
int main (int argc, char * argv [])
{
  ...
  /* Items counter */
  unsigned items   = _INITIALS_;             /* initial # of items per test */

  /* Loops counter */
  unsigned loops   = _LOOPS_;                /* # of loops per test         */

  unsigned choice  = OPT_EXECUTE;
  int option;
  ...
  if (files)
    {
      ...
      rsuite_t ** suite = build_suite (progname, enabled, disabled);
      if (suite)
	{
	  /* Build the subset of plugins and go! */
	  char ** subset = choose (progname, files, included, excluded);
	  if (subset)
	    doit (progname, _VERSION_, choice, dir, subset, suite, items, loops, verbose, quiet);
	  else
